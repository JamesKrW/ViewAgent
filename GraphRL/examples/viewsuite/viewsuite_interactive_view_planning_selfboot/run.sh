...
# sampling. Reuses the SLIME environment configs from the original
...
"${SLIME_PYTHON}" -m graphrl.main \
    ...
    general_overrides.rl.slime.train_envs="${ORIG_EXAMPLE_DIR}/train_turn_format.yaml" \
    general_overrides.rl.slime.eval_envs="${ORIG_EXAMPLE_DIR}/val.yaml" \
    iterations=4 \
    ...
    +iteration_overrides.iter3.rl.slime.train_envs="${ORIG_EXAMPLE_DIR}/train.yaml" \
    "$@" 2>&1 | tee "${LOG_FILE}"
