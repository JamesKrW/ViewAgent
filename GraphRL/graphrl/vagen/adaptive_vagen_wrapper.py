...
from graphrl.vagen.utils.command_builder import build_vagen_command, build_vagen_env, resolve_vagen_dir
...
class AdaptiveVagenWrapper(VagenWrapper):
    ...
    def launch(self) -> None:
        ...
        vagen_dir = resolve_vagen_dir(self.config)
        self._process = subprocess.Popen(
            cmd,
            cwd=str(vagen_dir),
            env=build_vagen_env(self.config),
    ...
    def _build_command(self, output_dir: Path) -> list[str]:
        ...
        # The module argument of `python -m <module>`. The adaptive entry point is
        # ViewAgent's now, not the backend's: VAGEN's own main_ppo_adaptive existed
        # only to pick a trainer class and went away with the old backend.
        cmd[2] = "graphrl.adaptive.entrypoint"
