"""The PPO trainer the adaptive controller runs, and only it.

Lives here rather than in the backend. The previous copy sat inside the VAGEN
fork as ``vagen/adaptive_ray_trainer.py``, so replacing the backend deleted the
experiment along with it. Nothing in it is VAGEN's: the schedule, the checkpoint
manifest and the rollout-prefix rules are all ``graphrl.adaptive``.

Hooks into four methods of ``VagenPPOTrainer``. Three are inherited from verl and
one is VAGEN's; where the older backend needed private attributes that no longer
exist, the replacement is noted at the override.
"""
...
class AdaptivePPOTrainer(VagenPPOTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not experiment_root or round_index is None:
            raise RuntimeError(
                "AdaptivePPOTrainer requires GRAPHRL_ADAPTIVE_EXPERIMENT_DIR "
    ...
    def _vagen_dump_images(self, batch) -> None:
        ...
        A single override now, because the backend separated the two: JSONL goes out
        through ``_fit_dump_data``'s super() call and images through here. The older
        backend wrote both from ``_log_rollout_data``, so suppressing images meant
        wrapping that method and toggling a private ``_log_image_enable`` around it.
        """
        if self._run_schedule.should_capture_rollout_images():
            return super()._vagen_dump_images(batch)
