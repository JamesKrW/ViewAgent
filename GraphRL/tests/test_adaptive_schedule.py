...
def _rollout_cfg(free_cache_engine):
    """A config stub exposing only what the release hook reads."""
    rollout = SimpleNamespace(
        get=lambda key, default=None: (
            free_cache_engine if key == "free_cache_engine" else default
        )
    )
...
def test_rollout_is_released_before_the_first_weight_sync():
    """The unpaired resume in SeparateRayPPOTrainer.fit must not reach sglang.

    fit() runs update_weights straight after _load_checkpoint, which resumes the
    `weights` tag; sglang implements resume as offload_tags.remove(tag) and dies
    with KeyError: 'weights' when nothing released first.
    """
    trainer = object.__new__(AdaptivePPOTrainer)
    trainer.config = _rollout_cfg(free_cache_engine=True)
    trainer.checkpoint_manager = _RecordingCheckpointManager()

    trainer._release_rollout_before_first_weight_sync()

    assert trainer.checkpoint_manager.sleeps == 1


def test_rollout_release_is_skipped_when_the_engine_never_sleeps():
    """With free_cache_engine off, verl issues no resume, so a release would be
    unpaired in the other direction -- sglang would then hold the tag forever."""
    trainer = object.__new__(AdaptivePPOTrainer)
    trainer.config = _rollout_cfg(free_cache_engine=False)
    trainer.checkpoint_manager = _RecordingCheckpointManager()

    trainer._release_rollout_before_first_weight_sync()

    assert trainer.checkpoint_manager.sleeps == 0
