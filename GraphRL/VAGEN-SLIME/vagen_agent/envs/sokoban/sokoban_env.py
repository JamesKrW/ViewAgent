...
from vagen_agent.envs._common import BaseCompactEnv, BaseConcatEnv, BaseNoConcatEnv

# from gym_sokoban.envs.sokoban_env import SokobanEnv
from vagen_agent.envs.sokoban.patch_sokoban_env import PatchedSokobanEnv as SokobanEnv
from vagen_agent.envs.sokoban.utils.prompt import (
    action_template,
    format_prompt,
...
from .utils.utils import numpy_to_pil, parse_response
...
class Sokoban(BaseNoConcatEnv, BaseConcatEnv, BaseCompactEnv):
    """
    ...
    def __init__(self, env_config: Dict[str, Any]):
        ...
        self._valid_turns = 0
        self._effective_turns = 0
        self._well_formed_turns = 0
    ...
    async def _step(
        ...
        """
        action_str = response.text if hasattr(response, "text") else str(response)
        ...
        action_is_valid = bool(
            len(action_list) > 0 and parsed.get("format_correct", False)
        )
        ...
        action_is_effective = not np.array_equal(
            prev_player_pos, self.env.player_position
        )

        self._metric_turns += 1
        self._valid_turns += int(action_is_valid)
        self._effective_turns += int(action_is_effective)
        self._well_formed_turns += int(bool(parsed.get("format_correct", False)))
        info["metrics"] = {
            "success": float(self._succeeded),
            "action_valid_rate": self._valid_turns / self._metric_turns,
            "action_effective_rate": self._effective_turns / self._metric_turns,
            "format_compliance": self._well_formed_turns / self._metric_turns,
        }
        info["success"] = self._succeeded
...
if __name__ == "__main__":
    ...
    async def main_async(render_mode: str = "vision",
        ...
        while True:
            ...
            obs, reward, terminated, truncated, info = await env.step(action_input)
            if "multi_modal_input" in obs:
                # save the image to target folder
                img = obs["multi_modal_input"][env.config.image_placeholder][0]
                img.save(os.path.join(save_path, f"step_{step}.png"))
            ...
            if terminated or truncated:
                print("Puzzle solved!" if terminated else "Episode truncated.")
                break
