...
def parse_free_think(response: str, action_sep: str = ",", max_actions: int = 3) -> Dict:
    """
    Parse free_think format response: reasoning, then ``<answer>...</answer>``.

    **The whole format requirement is a well-formed ``<answer>``.** Reasoning is free text
    and ``</think>`` is not required, because requiring it prices the format bonus
    differently per model family: a native-thinking checkpoint has the block opened for it
    by the chat template and closes it only when it is done, so on a tight per-turn budget
    it pays the bonus for stopping early rather than for writing the tags. What the format
    is meant to buy is a parseable action, and that is exactly ``<answer>``.

    Extraction stays two-tier: the strict tag decides ``format_correct``, and a loose
    search still salvages an action from a malformed turn rather than discarding the
    episode over punctuation.
    ...
    # Prefer an answer that follows the reasoning, but accept one anywhere: the tag is the
    # requirement, its position relative to `</think>` is not.
    ...
    native_box = _NATIVE_BOX_TAG.search(response) if after_close is None else None
    action_match = after_close or _ANSWER_TAG.search(response) or native_box
    format_correct = action_match is not None

    # `closed` may be -1 now that a well-formed answer alone is enough, and slicing to -1
    # would quietly drop the last character of the reasoning instead of keeping all of it.
