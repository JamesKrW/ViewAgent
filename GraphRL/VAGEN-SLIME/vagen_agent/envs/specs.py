...
# VENDORED CHANGE (vagen-slime): torch is optional here.
#
# Everything this port actually uses from this file -- EnvSpec, load_envspecs and the
# _generate_seeds_for_spec family -- is pure Python. Only AgenticDataset below needs
# torch, and this port does not use it: slime reads a jsonl, so the expansion happens
# offline in vagen_agent/make_dataset.py. Keeping the import hard would mean the dataset
# script could not run without a full training install.
#
# The rest of the file is verbatim from VAGEN, deliberately: the seed sequence a yaml
# expands into has to be identical on both stacks, or a "same config" comparison is
# comparing two different datasets.
try:
    from torch.utils.data import Dataset
except ModuleNotFoundError:  # pragma: no cover - only AgenticDataset needs it
    Dataset = object
...
try:
    import torch
except ModuleNotFoundError:  # pragma: no cover - see the note above; AgenticDataset only
    torch = None
...
class EnvSpec:
    ...
    # what an unconfigured environment was allowed to hand back: a flat 2048.
    #
    ...
    #
    # Model-agnostic here: this is a token count, and nothing in this port knows what a
    # reasoning block looks like. Which token closes one is engine config.
    #
    # NOT WIRED UP YET. sglang's equivalent is
    # ``sampling_params.custom_params["thinking_budget"]``, which makes
    # ``ReasonerGrammarObject`` allow only ``think_end_id`` once the budget is spent
    # (``srt/constrained/reasoner_grammar_backend.py``); it needs the engine started with
    # ``--sglang-reasoning-parser`` so ``think_end_id`` is known. That is the fix for a
    # native-thinking model on a tight per-turn budget -- raising the cap only buys more
    # reasoning, while this makes the model *stop* and answer. Setting the field today
    # does nothing.
